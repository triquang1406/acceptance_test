          ...
          if (!error) {
            ...
            try {
              ...
              devices.forEach((device) => {
                ...
                const BSDName = util.sanitizeShellString(util.getValue(lines, 'BSD Name', ':', true).trim(), true);
                if (sizeStr) {
                  let sizeValue = 0;
                ...
                if (sizeStr) {
                  ...
                  if (sizeValue) {
                    ...
                    result.push({
                      device: BSDName,
                      type: 'USB',
                      name: util.getValue(lines, 'Model', ':', true).trim().replace(/:/g, ''),
                      vendor: getVendorFromModel(util.getValue(lines, 'Model', ':', true).trim()),
                      size: sizeValue,
                      bytesPerSector: null,
                      totalCylinders: null,
            ...
            devices.forEach((device) => {
              ...
              if (hasControlCharacters(serialNum)) {
                ...
                if (instanceId) {
                  ...
                  // instanceId can contain device serial number or be generated by PnP manager
